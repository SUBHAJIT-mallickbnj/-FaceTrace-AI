...
if not is_user_authenticated(st.session_state):
    ...
    user = st.session_state.get("user") or st.session_state.get("username")
...
if user is not None:
    ...
    with image_col:
        ...
        if image_obj:
            ...
            if st.session_state.get("nc_file_key") != file_key:
                ...
                uploaded_file_path = get_resources_dir() / f"{upload_id}.jpg"
            ...
            if not faces:
                ...
                render_image(annotated, width="stretch")
            ...
            if submit_bt:
                ...
                if errors:
                    ...
                    new_case_details = RegisteredCases(
                        ...
                        face_mesh=json.dumps(selected_feature),
                        adhaar_card=adhaar_card.strip(),
                        birth_marks=birthmarks.strip(),
                        address=address.strip(),
                        city=city.strip() or None,
                        latitude=geocode_location(city, last_seen, address)[0],
                        longitude=geocode_location(city, last_seen, address)[1],
                    ...
                    if upload_id and case_id:
                        old_image_path = get_resources_dir() / f"{upload_id}.jpg"
                        new_image_path = get_resources_dir() / f"{case_id}.jpg"
    ...
    st.warning("Please log in from the Home page before accessing this page.")
