...
def case_viewer(case, is_admin: bool = False):
    ...
    if image_path:
        render_image(str(image_path), width=120)
    else:
...
if "login_status" not in st.session_state:
    ...
    status = filter_col.selectbox(
        "Filter by Status",
        options=["All", "Not Found", "Found", "Public Cases Only"],
    ...
    # ── Public Cases Only view ────────────────────────────────────────────────
    if status == "Public Cases Only":
        cases_data = list(db_queries.fetch_public_cases(train_data=False))
    ...
    # ── Registered Cases view ─────────────────────────────────────────────────
        ...
        # ── Also show Public Sightings below Registered Cases ──────────────
        st.write("---")
        st.subheader("👁️ Public Sightings to Review")
        public_cases_data = list(db_queries.fetch_public_cases(train_data=False))
        
        if search_name:
            public_cases_data = [
                c for c in public_cases_data if search_name.lower() in str(c).lower()
            ]
        
        if date_filter:
            public_cases_data = [
                c for c in public_cases_data if c[5] and str(c[5])[:10] >= str(date_filter)
            ]
        
        if not public_cases_data:
            st.info("No public sightings found.")
        else:
            paginated_pub, render_controls_pub = paginate(public_cases_data, "page_pub")
            for case in paginated_pub:
                public_case_viewer(case)
            render_controls_pub()
